...
import { phonesApi, marquesApi, categoriesApi } from '@/lib/services';
import type { Phone, Marque, Categorie } from '@/types';
import { formatPrice } from '@/lib/format';
import PageHeader from '@/components/ui/PageHeader';
import SearchInput from '@/components/ui/SearchInput';
import Modal from '@/components/ui/Modal';
import ConfirmDialog from '@/components/ui/ConfirmDialog';
import StatusBadge, { AVAILABILITY_OPTIONS } from '@/components/ui/StatusBadge';
...
export default function Phones() {
  ...
  function openAdd() {
    setEditing(null);
    setForm({
      image: '',name: '', couleur: '', prix: '', stockage: '', etat: '',
      id_marque: '', id_categorie: '', disponibilite: 'En Stock',
    });
    setModalOpen(true);
  }
  ...
  return (
    <div>
      ...
      <div className="mb-4 max-w-md">
        <SearchInput value={search} onChange={setSearch} placeholder="Rechercher par nom, marque, catégorie..." />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <EmptyState title="Aucun téléphone trouvé" message="Ajoutez votre premier téléphone pour commencer" />
        ) : (
                  ...
                  <tr key={phone.id} className="transition-colors hover:bg-slate-50/50">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-3">
                        <img src={phone.image} alt={phone.name} className="h-10 w-10 rounded-lg object-cover"/>
                    ...
                    <td className="px-5 py-3.5 font-medium text-slate-700">{formatPrice(phone.prix)}</td>
                    ...
                    <td className="px-5 py-3.5"><StatusBadge status={phone.disponibilite} /></td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => { setViewPhone(phone); setViewOpen(true); }}
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <FormField label="Nom" name="name" value={form.name} onChange={(v) => setForm({ ...form, name: v })} required placeholder="Ex: Galaxy S24 Ultra" />
          </div>
          <FormField label="Image" name="image" value={form.image} onChange={(v) => setForm({ ...form, image: v })} placeholder="images/iphone16.jpg"/>
          <FormField label="Marque" name="id_marque" value={form.id_marque} onChange={(v) => setForm({ ...form, id_marque: v })} options={marques.map((m) => ({ value: m.id, label: m.nom }))}/>
          <FormField label="Catégorie" name="id_categorie" value={form.id_categorie} onChange={(v) => setForm({ ...form, id_categorie: v })} options={categories.map((c) => ({ value: c.id, label: c.nom }))}/>
          <FormField label="Prix (ARIARY)" name="prix" type="number" value={form.prix} onChange={(v) => setForm({ ...form, prix: v })} placeholder="0" />
          <FormField label="Couleur" name="couleur" value={form.couleur} onChange={(v) => setForm({ ...form, couleur: v })} placeholder="Ex: Noir" />
          <FormField label="Stockage" name="stockage" value={form.stockage} onChange={(v) => setForm({ ...form, stockage: v })} placeholder="Ex: 256 Go" />
          <FormField label="État" name="etat" value={form.etat} onChange={(v) => setForm({ ...form, etat: v })} options={ETAT_OPTIONS.map((e) => ({ value: e, label: e }))}/>
          <FormField label="Disponibilité" name="disponibilite" value={form.disponibilite} onChange={(v) => setForm({ ...form, disponibilite: v })} options={AVAILABILITY_OPTIONS.map((a: string) => ({ value: a, label: a }))}/>
        </div>
      ...
      {/* View Modal */}
      <Modal open={viewOpen} onClose={() => setViewOpen(false)} title="Détails du téléphone" size="md">
        {viewPhone && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 rounded-xl bg-blue-50 p-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{viewPhone.marque?.nom || '-'} · {viewPhone.categorie?.nom || '-'}</p>
              </div>
            </div>
            ...
            <div>
              <p className="mb-1.5 text-xs font-medium uppercase tracking-wide text-slate-400">Disponibilité</p>
