...
import { marquesApi } from '@/lib/services';
...
export default function Marques() {
  ...
  function openEdit(m: Marque) { setEditing(m); setNom(m.nom); setModalOpen(true); }
  ...
  return (
    <div>
      <PageHeader title="Marques" description="Gérez les marques de téléphones" onAdd={openAdd} addLabel="Ajouter une marque" />
      <div className="mb-4 max-w-md"><SearchInput value={search} onChange={setSearch} placeholder="Rechercher une marque..." /></div>
                  ...
                  <tr key={m.id} className="transition-colors hover:bg-slate-50/50">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-50"><Tag size={16} className="text-purple-600" /></div>
                        <span className="font-medium text-slate-700">{m.nom}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(m)} className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-amber-50 hover:text-amber-600" title="Modifier"><Pencil size={17} /></button>
                        <button onClick={() => setDeleteId(m.id)} className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600" title="Supprimer"><Trash2 size={17} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Modifier la marque' : 'Ajouter une marque'}>
        <FormField label="Nom" name="nom" value={nom} onChange={setNom} required placeholder="Ex: Samsung" />
      ...
      <ConfirmDialog open={deleteId != null} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Supprimer la marque" message="Êtes-vous sûr de vouloir supprimer cette marque ?" />
    </div>
  );
}
