...
import { phonesApi, reservationsApi, contactsApi, servicesApi, blogsApi } from '@/lib/services';
import type { Phone, Reservation, Contact } from '@/types';
...
export default function Dashboard() {
  ...
  const statCards = [
    { title: 'Téléphones', value: stats.phones, icon: Smartphone, color: 'text-blue-600', bgColor: 'bg-blue-50', link: '/phones' },
    { title: 'Réservations', value: stats.reservations, icon: CalendarCheck, color: 'text-emerald-600', bgColor: 'bg-emerald-50', link: '/reservations' },
    { title: 'Messages', value: stats.contacts, icon: Mail, color: 'text-amber-600', bgColor: 'bg-amber-50', link: '/contacts' },
    { title: 'Services', value: stats.services, icon: Wrench, color: 'text-purple-600', bgColor: 'bg-purple-50', link: '/services' },
    { title: 'Articles blog', value: stats.blogs, icon: Newspaper, color: 'text-rose-600', bgColor: 'bg-rose-50', link: '/blogs' },
  ];
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              ...
              <Link to="/phones" className="text-xs font-medium text-blue-600 hover:underline">
                Tout voir
              </Link>
            </div>
              ...
              {recentPhones.length === 0 ? (
                ...
                recentPhones.map((phone) => (
                  <div key={phone.id} className="flex items-center justify-between px-5 py-3">
                    ...
                    <span className="text-sm font-semibold text-slate-600">{formatPrice(phone.prix)}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              ...
              <Link to="/reservations" className="text-xs font-medium text-blue-600 hover:underline">
                Tout voir
              </Link>
            </div>
            <div className="divide-y divide-slate-50">
              {recentReservations.length === 0 ? (
                ...
                recentReservations.map((res) => (
                  <div key={res.id} className="flex items-center justify-between px-5 py-3">
                    <div>
                      <p className="text-sm font-medium text-slate-700">{res.client?.nom || 'Client'}</p>
                ...
                recentContacts.map((contact) => (
                  <div key={contact.id} className="px-5 py-3">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-slate-700">{contact.nom || 'Anonyme'}</p>
      ...
      <div className="mt-8 flex items-center gap-4 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-sky-50 p-6">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            Gérez vos téléphones, services Canal+, réservations et plus encore depuis une interface unique.
          </p>
        </div>
      </div>
    </div>
