...
export default function Contacts() {
  ...
  return (
    <div>
      <PageHeader title="Messages de contact" description="Les messages reçus via le formulaire de contact" />

      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? <LoadingSpinner /> : contacts.length === 0 ? (
          <EmptyState title="Aucun message trouvé" message="Les messages de contact apparaîtront ici" />
        ) : (
                  ...
                  <tr key={c.id} className="transition-colors hover:bg-slate-50/50">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-50"><User size={16} className="text-amber-600" /></div>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => { setViewItem(c); setViewOpen(true); }} className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-blue-50 hover:text-blue-600" title="Voir"><Eye size={17} /></button>
      ...
      <Modal open={viewOpen} onClose={() => setViewOpen(false)} title="Détails du message" size="md">
        {viewItem && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 rounded-xl bg-amber-50 p-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-amber-500"><Mail size={28} className="text-white" /></div>
              <div>
      ...
      <ConfirmDialog open={deleteId != null} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Supprimer le message" message="Êtes-vous sûr de vouloir supprimer ce message ?" />
    </div>
  );
}
