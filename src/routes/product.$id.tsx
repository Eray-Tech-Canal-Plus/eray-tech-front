      ...
      <div className="bg-secondary text-secondary-foreground text-xs">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-2.5">
          ...
          <span className="hidden md:block">
            Inscrivez-vous et obtenez <span className="text-primary font-semibold">20% de réduction</span>.{" "}
            <button onClick={() => toast.success("Inscription ouverte !")} className="underline">Inscription</button>
          </span>
      ...
      <header className="border-b border-border bg-background sticky top-0 z-30">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          ...
          <div className="flex items-center gap-4">
            <Search className="h-5 w-5 cursor-pointer hover:text-primary" onClick={() => navigate({ to: "/" })} />
            <Heart className="h-5 w-5 cursor-pointer hover:text-primary" onClick={() => toast("Favoris")} />
            <ShoppingBag className="h-5 w-5 cursor-pointer hover:text-primary" onClick={() => toast("Panier")} />
            <User className="h-5 w-5 cursor-pointer hover:text-primary" onClick={() => toast("Compte")} />
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl px-6 py-12 grid grid-cols-1 md:grid-cols-2 gap-10">
        <div>
          <div className="relative aspect-square rounded-xl overflow-hidden bg-muted">
            ...
            <button onClick={prevImg} className="absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-background/90 flex items-center justify-center shadow hover:bg-primary hover:text-primary-foreground">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={nextImg} className="absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${product.inStock ? "bg-accent text-primary" : "bg-secondary text-secondary-foreground"}`}>
          ...
          <div className="flex items-center gap-2 mt-3">
            <div className="flex">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`h-4 w-4 ${i < Math.floor(product.rating) ? "fill-primary text-primary" : "text-muted-foreground/40"}`} />
              ))}
            </div>
            <span className="text-sm">{product.rating} ({product.reviews} avis)</span>
          </div>
          <div className="mt-4 flex items-baseline gap-3">
            ...
            <span className="text-xs font-semibold bg-primary text-primary-foreground px-2 py-0.5 rounded">-{product.discount}%</span>
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <div className="flex items-center border border-border rounded-full">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="h-10 w-10 flex items-center justify-center hover:text-primary"><Minus className="h-4 w-4" /></button>
              <span className="w-10 text-center font-semibold">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="h-10 w-10 flex items-center justify-center hover:text-primary"><Plus className="h-4 w-4" /></button>
            </div>
            <button
            ...
            <button onClick={() => toast.success("Ajouté aux favoris")} className="h-10 w-10 border border-border rounded-full flex items-center justify-center hover:text-primary">
          ...
          <div className="mt-6 text-sm space-y-1.5">
            ...
            <p><span className="text-muted-foreground">Tags :</span> <span className="font-medium">{product.brand}, {product.category}, Smartphone</span></p>
            <div className="flex items-center gap-3">
              <span className="text-muted-foreground">Partager :</span>
              {[Facebook, Twitter, Instagram, Share2].map((Icon, i) => (
                <button key={i} onClick={() => toast("Lien copié")} className="h-8 w-8 rounded-full bg-accent text-primary flex items-center justify-center hover:bg-primary hover:text-primary-foreground">
                ...
                {REVIEWS.map((r, i) => (
                  <div key={i} className="border-b border-border pb-6">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-accent text-primary flex items-center justify-center font-semibold">{r.name[0]}</div>
                        <div>
                          <p className="font-medium text-sm">{r.name}</p>
                          <p className="text-xs text-muted-foreground">{r.verified ? "Vérifié" : ""}</p>
                        </div>
                      </div>
...
function LinkMobileCheckout({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-background rounded-2xl w-full max-w-md p-6 relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute right-4 top-4 h-8 w-8 rounded-full hover:bg-muted flex items-center justify-center">
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="rounded-lg bg-muted p-4 mb-5 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">Produit</span><span className="font-medium">{product.name}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Options</span><span className="font-medium">{storage} · {color}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Quantité</span><span className="font-medium">{qty}</span></div>
          <div className="flex justify-between pt-2 mt-2 border-t border-border"><span className="font-semibold">Total</span><span className="font-bold text-primary">{total} €</span></div>
        </div>
        ...
        {step === "form" && (
          <form onSubmit={submit} className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground flex items-start gap-2">
              <Shield className="h-3.5 w-3.5 mt-0.5 text-primary" /> Vos informations LinkMobile sont chiffrées de bout en bout.
            </p>
        ...
        {step === "confirm" && (
          <div className="text-center space-y-4">
            <p className="text-sm">Un code de confirmation a été envoyé au <b>{phone}</b>. Confirmez sur votre application LinkMobile.</p>
        ...
        {step === "success" && (
          <div className="text-center space-y-4 py-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Votre commande de {total} € a été confirmée. Vous recevrez un SMS LinkMobile de suivi.</p>
            </div>
            <button onClick={onClose} className="w-full h-11 rounded-full bg-primary text-primary-foreground font-semibold">
