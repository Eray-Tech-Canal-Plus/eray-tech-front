...
import { products, CATEGORIES, BRANDS, STORAGES, COLORS, type Product } from "@/lib/products";
...
function ShopInner() {
  ...
  if (priceMax < PRICE_MAX)
    activeChips.push({ label: `Jusqu'à ${priceMax} €`, onRemove: () => setPriceMax(PRICE_MAX) });
  selCats.forEach((c) =>
    activeChips.push({ label: c, onRemove: () => setSelCats((v) => v.filter((x) => x !== c)) }),
  );
  selBrands.forEach((c) =>
    activeChips.push({ label: c, onRemove: () => setSelBrands((v) => v.filter((x) => x !== c)) }),
  );
  selStorages.forEach((c) =>
    activeChips.push({ label: c, onRemove: () => setSelStorages((v) => v.filter((x) => x !== c)) }),
  );
  selColors.forEach((c) =>
    activeChips.push({ label: c, onRemove: () => setSelColors((v) => v.filter((x) => x !== c)) }),
  );
  selRatings.forEach((c) =>
    activeChips.push({
      label: `${c}+ étoiles`,
      onRemove: () => setSelRatings((v) => v.filter((x) => x !== c)),
    }),
  );
      ...
      <div className="bg-secondary text-secondary-foreground text-xs">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-2.5">
          ...
          <span className="hidden md:block">
            ...
            <span className="text-primary font-semibold">20% de réduction</span> sur votre première
            commande.{" "}
            <button
      ...
      <header className="border-b border-border bg-background sticky top-0 z-30">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 gap-6">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            <button onClick={() => toast("Page à venir")} className="hover:text-primary transition">
              À propos
            </button>
          </nav>
          <div className="flex items-center gap-3 text-foreground">
            ...
            <button onClick={() => toast("Favoris mis à jour")} className="hover:text-primary">
              <Heart className="h-5 w-5" />
            </button>
            <button onClick={() => toast("Panier ouvert")} className="hover:text-primary">
              <ShoppingBag className="h-5 w-5" />
            </button>
            <button onClick={() => toast("Connexion à venir")} className="hover:text-primary">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-10">
        ...
        <aside className="space-y-8">
          ...
          <FilterBlock
            ...
            selected={selBrands}
            onToggle={(v) => {
              setSelBrands(toggle(selBrands, v));
              setPage(1);
            }}
          />
          ...
          <FilterBlock
            ...
            onToggle={(v) => {
              setSelStorages(toggle(selStorages, v));
              setPage(1);
            }}
          />

          <div>
            <h3 className="text-sm font-semibold mb-4 border-l-2 border-primary pl-3">
              Note client
            </h3>
              ...
              {[5, 4, 3, 2, 1].map((n) => (
                <label key={n} className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    ...
                    onChange={() => {
                      setSelRatings(toggle(selRatings, n));
                      setPage(1);
                    }}
          ...
          <FilterBlock
            ...
            selected={selColors}
            onToggle={(v) => {
              setSelColors(toggle(selColors, v));
              setPage(1);
            }}
          />
        ...
        <main>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <p className="text-sm text-muted-foreground">
              Affichage {filtered.length === 0 ? 0 : (currentPage - 1) * PER_PAGE + 1}–
              {Math.min(currentPage * PER_PAGE, filtered.length)} sur {filtered.length} résultats
            </p>
          ...
          {activeChips.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 mb-8">
              ...
              <button
                onClick={clearAll}
                className="text-xs text-primary underline underline-offset-4 ml-2"
              >
                Tout effacer
              </button>
            </div>
          ...
          {pageItems.length === 0 ? (
            <div className="text-center py-20 text-muted-foreground">
              ...
              <div>
                <button onClick={clearAll} className="text-primary underline mt-3">
                  Réinitialiser
                </button>
              </div>
            </div>
      ...
      <section className="border-t border-border">
        <div className="mx-auto max-w-7xl px-6 py-10 grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            {
              icon: CreditCard,
              title: "Paiement LinkMobile",
              desc: "Payez en toute sécurité via LinkMobile",
            },
            {
              icon: Headphones,
              title: "Support 24 × 7",
              desc: "Notre équipe vous accompagne à tout moment",
            },
          ].map((p) => (
      ...
      <footer className="bg-secondary text-secondary-foreground/80 text-sm">
        <div className="mx-auto max-w-7xl px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-6">
            <button onClick={() => toast("Mentions légales")} className="hover:text-primary">
              Mentions légales
            </button>
            <button onClick={() => toast("Confidentialité")} className="hover:text-primary">
              Confidentialité
            </button>
            <button
              onClick={() => toast("Contact : contact@phonelux.fr")}
              className="hover:text-primary"
            >
              Contact
            </button>
