...
import { blogsApi } from "@/admin/services";
...
export default function Blogs() {
  ...
  async function handleSave() {
    if (!form.titre.trim()) {
      notify("Le titre est obligatoire", "warning");
      return;
    }
    setSaving(true);
    try {
                  ...
                  <tr key={b.id} className="transition-colors hover:bg-slate-50/50">
                    <td className="px-5 py-3.5">
                      {b.image ? (
                        ...
                        <div className="flex h-12 w-16 items-center justify-center rounded-lg bg-rose-50">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField
              ...
              onChange={(v) => setForm({ ...form, date_publication: v })}
            />
          </div>
          <FormField
            label="Image (URL)"
          ...
          <div>
            ...
            <RichTextEditor
              value={form.contenu}
              onChange={(v) => setForm({ ...form, contenu: v })}
